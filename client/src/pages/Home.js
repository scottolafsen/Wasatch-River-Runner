...
class Home extends Component {
  ...
  setGauges() {
    axios
      .get(
        "https://waterservices.usgs.gov/nwis/iv?format=json&sites=10016900,10092700,10168000,10109000,10140100,10140700,10154200,10149000,10137500,10132000,10136500,10137000,10163000&parameterCd=00060&period=P7D"
      )
      .then(res => {
              ...
              {this.state.dashRivers.map(river => {
                return (
                  <RiverTableItem
                    ...
                    updated={
                      river.updated !== "" ? (
                        <Moment fromNow>{river.updated}</Moment>
                      ) : (
                        "no gauge"
                      )
                    }
